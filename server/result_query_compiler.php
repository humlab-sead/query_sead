...
class ResultQueryCompiler {
    ...
    private static function createResultQueryConfig($result_definition, $facetConfig, $resultConfig)
    {
    ...
    */
    public static function compileQuery($facetConfig, $resultConfig)
    {
        ...
        $queryConfig = self::createResultQueryConfig($result_definition, $facetConfig, $resultConfig);

        if (empty($queryConfig) || empty($queryConfig["data_fields"])) {
            return "";
        }

        $query = QueryBuildService::compileQuery2($facetConfig, $facetCode, $queryConfig["data_tables"]);
